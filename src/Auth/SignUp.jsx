  ...
  return (
    <div className='w-[100%] h-[100vh] flex justify-center items-center'>
      <div className='w-[30%] h-[90%] bg-[#FDFDF7] rounded-[10px] shadow-xl max-sm:w-[90%]'>
        ...
        <form onSubmit={handleSubmit(onSubmit)} className='w-[100%] h-[80%] max-sm:h-[80%] '>
          {/* Company Name Input */}
          <div className='w-[100%] h-[17%] flex flex-col justify-center items-start p-2'>
            <label htmlFor='company_Name'>Company Name</label>
            <input type='text' {...register('company_Name')} className='w-[100%] h-[90%] rounded-md outline-none border-solid border-[#023047] border bg-transparent p-2' onChange={(e)=> setcompany_Name(e.target.value)} />
            {errors.company_Name && <p className='text-red-500 text-[10px]'>{errors.company_Name.message}</p>}
          </div>
          {/* Email Input */}
          <div className='w-[100%] h-[17%] flex flex-col justify-center items-start p-2'>
            <label htmlFor='email'>Email</label>
            <input type='text' {...register('email')} className='w-[100%] h-[90%] rounded-md outline-none border-solid border-[#023047] border bg-transparent p-2' onChange={(e)=> setemail(e.target.value)} />
            {errors.email && <p className='text-red-500 text-[10px]'>{errors.email.message}</p>}
          </div>
          {/* Password Input */}
          <div className='w-[100%] h-[17%] flex flex-col justify-center items-start p-2'>
            <label htmlFor='password'>Password</label>
            <input type='password' {...register('password')} className='w-[100%] h-[90%] rounded-md outline-none border-solid border-[#023047] border bg-transparent p-2' onChange={(e)=> setpassword(e.target.value)} />
            {errors.password && <p className='text-red-500 text-[10px]'>{errors.password.message}</p>}
          </div>
          {/* Confirm Password Input */}
          <div className='w-[100%] h-[17%] flex flex-col justify-center items-start p-2'>
            <label htmlFor='confirmPassword'>Confirm Password</label>
            <input type='password' {...register('confirmPassword')} className='w-[100%] h-[90%] rounded-md outline-none border-solid border-[#023047] border bg-transparent p-2' onChange={(e)=> setconfirmPassword(e.target.value)} />
          ...
          <section className='w-[100%] h-[18%] flex justify-center items-center flex-col'>
            ...
            <span className='font-semibold text-[#023047]'>
              Already have an account ? <Link to='/login' className='text-md text-[#FB8500] font-bold'>Login</Link>
            </span>
          </section>
        </form>
      </div>
